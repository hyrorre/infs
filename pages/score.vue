...
const loadChartstats = (force = false) => {
  ...
  // }
  loading.value = true
  fetch('https://infs.iidx.wiki/api/getscore/hyrorre', {
    method: 'GET',
    headers: {
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
      <div class="mb-2">
        <div>
          ...
          <span v-for="style in ['SP', 'DP']" class="inline-block ml-2 mb-2">
            <button class="btn-primary" v-if="filter.style === style">{{ style }}</button>
            <button class="btn-secondary" v-else @click="filter.style = style">{{ style }}</button>
          </span>
        </div>
        <div>
          <span class="align-middle">DIFFICULTY</span>
          <span v-for="difficulty in ['B', 'N', 'H', 'A', 'L']" class="inline-block ml-2 mb-2">
            <button class="btn-primary" v-if="filter.difficulty === difficulty" @click="filter.difficulty = null">
              {{ difficulty }}
            </button>
            <button class="btn-secondary" v-else @click="filter.difficulty = difficulty">{{ difficulty }}</button>
          </span>
        </div>
        <div>
          <span class="align-middle">LEVEL</span>
          <span v-for="level in [...Array(12)].map((_, i) => i + 1)" class="inline-block ml-2 mb-2">
            <button class="btn-primary" v-if="filter.level === level" @click="filter.level = null">{{ level }}</button>
            <button class="btn-secondary" v-else @click="filter.level = level">{{ level }}</button>
          </span>
        </div>
        <div>
          <span class="align-middle">LAMP</span>
          <span v-for="lamp in ['FC', 'EX', 'HC', 'NC', 'EC', 'AC', 'NP']" class="inline-block ml-2 mb-2">
            <button class="btn-primary" v-if="filter.lamp === lamp" @click="filter.lamp = null">{{ lamp }}</button>
            <button class="btn-secondary" v-else @click="filter.lamp = lamp">{{ lamp }}</button>
          </span>
        </div>
        <div>
          <span class="align-middle">GRADE</span>
          <span v-for="grade in ['AAA', 'AA', 'A', 'B', 'C', 'D', 'E', 'F']" class="inline-block ml-2 mb-2">
            <button class="btn-primary" v-if="filter.grade === grade" @click="filter.grade = null">{{ grade }}</button>
            <button class="btn-secondary" v-else @click="filter.grade = grade">{{ grade }}</button>
          </span>
        </div>
        <div>
          ...
          >
            <button class="btn-primary" v-if="sort === s" @click="reverse = !reverse">
              {{ s + ' ' + (reverse ? '▼' : '▲') }}
            </button>
            <!-- prettier-ignore -->
            <button class="btn-secondary" v-else @click="sort = s; reverse = false">
                {{ s }}
              </button>
          </span>
        </div>
      ...
      <div class="flex justify-between items-end mb-2">
        ...
        <div>
          ...
          >
            <button class="btn-secondary" class="mr-2">Share</button>
          </a> -->
          <button class="btn-secondary" @click="loadChartstats(true)">Reload</button>
        </div>
      </div>
      <div class="bg-white dark:bg-gray-800 overflow-x-scroll shadow-xl sm:rounded-lg">
        <p class="p-4" v-if="isPrivate">{{ 'Rival' }}'s Score is private.</p>
